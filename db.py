...
def connect_db():
    ...
    return sqlite3.connect('AirlineNewNew.db')
...
class AirlineApp:
    ...
    def add_booking(self):
        ...
        try:
            ...
            cursor.execute("INSERT INTO Bookings (flight_id, passenger_id, booking_date) VALUES (?, ?, ?)",
    ...
    def update_flight(self):
        if hasattr(self, 'edit_flight_id'):
            ...
            try:
                cursor.execute(
                    """UPDATE Flights SET flight_number=?, departure_city=?, arrival_city=?, departure_time=?, arrival_time=? WHERE flight_id=?""",
    ...
    def delete_selected_aircraft(self):
        ...
        if selected_item:
            ...
            if messagebox.askyesno("Confirm Deletion", "Are you sure you want to delete this aircraft?"):
                conn = connect_db()
                cursor = conn.cursor()
                cursor.execute("DELETE FROM Aircrafts WHERE aircraft_id=?", (aircraft_id,))
                conn.commit()
                conn.close()
                self.view_aircraft()
        else:
    ...
    def delete_selected_crew_member(self):
        ...
        if selected_item:
            ...
            if messagebox.askyesno("Confirm Deletion", "Are you sure you want to delete this crew member?"):
                conn = connect_db()
                cursor = conn.cursor()
                cursor.execute("DELETE FROM Crew_Members WHERE crew_id=?", (crew_id,))
                conn.commit()
                conn.close()
                self.view_crew()
        else:
